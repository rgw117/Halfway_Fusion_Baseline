...
def pytorch_normalze(vis_img, lwir_img):
    ...
    normalize_lwir = tvtsf.Normalize(mean=[0.1598],std=[0.0813])                  
    lwir_img = normalize_lwir(t.from_numpy(lwir_img).float())

    return vis_img.numpy(), lwir_img.numpy()


def preprocess(vis_img, lwir_img, min_size=600, max_size=1000):
    ...
    """
    # C_vis, H_vis, W_vis = vis_img.shape
    # C_lwir, H_lwir, W_lwir = lwir_img.shape

    # scale1 = min_size / min(H_vis, W_vis)
    # scale2 = max_size / max(H_vis, W_vis)
    ...
    # vis_img = sktsf.resize(vis_img, (C_vis, H_vis * scale, W_vis * scale), mode='reflect',anti_aliasing=False)
    # lwir_img = sktsf.resize(lwir_img, (C_lwir, H_lwir * scale, W_lwir * scale), mode='reflect',anti_aliasing=False)

    vis_img,lwir_img = pytorch_normalze(vis_img,lwir_img)

    return vis_img,lwir_img
...
#         ori_img, bbox, label, difficult = self.db.get_example(idx)
        ...
        vis_ori_img = np.array(vis_ori_img).transpose(2,0,1)
...
class TestDataset:
    ...
    def __getitem__(self, idx):
        ...
        vis_ori_test_img = np.array(vis_ori_test_img).transpose(2,0,1)
