...
class VGG_vis_lwir_feature(nn.Module):
    ...
    def forward(self, vis_image, lwir_image):
        ...
        out_vis = F.relu(self.conv1_1_vis(vis_image.cuda()))  # (N, 64, 512, 640)
        out_vis = F.relu(self.conv1_2_vis(out_vis))  # (N, 64, 512, 640)
        out_vis = self.pool1_vis(out_vis)  # (N, 64, 256, 320)

        out_vis = F.relu(self.conv2_1_vis(out_vis))  # (N, 128, 256, 320)
        out_vis = F.relu(self.conv2_2_vis(out_vis))  # (N, 128, 256, 320)
        out_vis = self.pool2_vis(out_vis)  # (N, 128, 128, 160)

        out_vis = F.relu(self.conv3_1_vis(out_vis))  # (N, 256, 128, 160)
        out_vis = F.relu(self.conv3_2_vis(out_vis))  # (N, 256, 128, 160)
        out_vis = F.relu(self.conv3_3_vis(out_vis))  # (N, 256, 128, 160)
        out_vis = self.pool3_vis(out_vis)  # (N, 256, 64, 80)

        out_vis = F.relu(self.conv4_1_vis(out_vis))  # (N, 512, 64, 80)
        out_vis = F.relu(self.conv4_2_vis(out_vis))  # (N, 512, 64, 80)
        
        out_vis = F.relu(self.conv4_3_vis(out_vis))  # (N, 512, 64, 80)
        ...
        concatenate_feature = t.cat((out_vis,out_lwir), 1) # (N, 1024, 64, 80)

        # Network In Network
        concatenate_feature = F.avg_pool2d(concatenate_feature, kernel_size=3, stride=1, padding=1) # (N, 1024, 64, 80)
        concatenate_feature = F.relu(self.conv1x1(concatenate_feature)) # (N, 512, 64, 80)

        concatenate_feature = F.relu(self.conv5_1(concatenate_feature))  # (N, 512, 64, 80)
        concatenate_feature = F.relu(self.conv5_2(concatenate_feature))  # (N, 512, 64, 80)
        concatenate_feature = F.relu(self.conv5_3(concatenate_feature))  # (N, 512, 64, 80)
        concatenate_feature = self.pool5(concatenate_feature)  # (N, 512, 32, 40)
    ...
    def load_pretrained_layers(self):
        ...
        # Transfer conv. parameters from pretrained model to current model
        for i, param in enumerate(param_names[20:-8]):  # excluding conv6 and conv7 parameters            
        ...
        # Transfer conv. parameters from pretrained model to current model
        for i, param in enumerate(param_names[-6:]):  # excluding conv6 and conv7 parameters            
            state_dict[param] = pretrained_state_dict[pretrained_param_names[i+20]]
        ...
        print("\nLoaded base vis&lwir_model.\n")

'''
class VGG_lwir_feature(nn.Module):
    """
    VGG base convolutions to produce lower-level feature maps.
    """

    def __init__(self):
        super(VGG_lwir_feature, self).__init__()

        # Standard convolutional layers in VGG16
        self.conv1_1 = nn.Conv2d(1, 64, kernel_size=3, padding=1, bias=True)  # stride = 1, by default
    ...
    def forward(self, image):
        ...
        :return: feature
        """
        out = F.relu(self.conv1_1(image))   # (N, 64, 512, 640)
        out = F.relu(self.conv1_2(out))   # (N, 64, 512, 640)
        out = self.pool1(out)  # (N, 64, 256, 320)

        out = F.relu(self.conv2_1(out))  # (N, 128, 256, 320)
        out = F.relu(self.conv2_2(out))  # (N, 128, 256, 320)
        out = self.pool2(out)  # (N, 128, 128, 160)

        out = F.relu(self.conv3_1(out))  # (N, 256, 128, 160)
        out = F.relu(self.conv3_2(out))  # (N, 256, 128, 160)
        out = F.relu(self.conv3_3(out))  # (N, 256, 128, 160)
        out = self.pool3(out)  # (N, 256, 64, 80)

        out = F.relu(self.conv4_1(out))  # (N, 512, 64, 80)
        out = F.relu(self.conv4_2(out))  # (N, 512, 64, 80)
        feature = F.relu(self.conv4_3(out))  # (N, 512, 64, 80)
...
class VGG_vis_lwir_feature(nn.Module):
    ...
    def __init__(self):
        ...
        self.vis_feature = VGG_vis_feature()
        self.lwir_feature = VGG_lwir_feature()
    ...
    def forward(self, vis_image, lwir_image):
        """
        Forward propagation.
        input : vis_feature , lwir_feature
        output : feature
        """
        vis_feature = self.vis_feature(vis_image)
        lwir_feature = self.lwir_feature(lwir_image)

        concatenate_feature = t.cat((vis_feature,lwir_feature), 1) # (N, 1024, 64, 80)

        # Network In Network
        out = F.avg_pool2d(concatenate_feature, kernel_size=3, stride=1, padding=1) # (N, 1024, 64, 80)
        out = F.relu(self.conv1x1(out)) # (N, 512, 64, 80)

        out = F.relu(self.conv5_1(out))  # (N, 512, 64, 80)
        out = F.relu(self.conv5_2(out))  # (N, 512, 64, 80)
        out = F.relu(self.conv5_3(out))  # (N, 512, 64, 80)
        feature = self.pool5(out)  # (N, 512, 32, 40)
    ...
    def load_pretrained_layers(self):
            ...
            print("Loaded base vis_lwir_model.\n")
'''
...
class VGG_vis_lwir_classifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.Linear1 = nn.Linear(in_features=512*32*40, out_features=4096, bias=True)
    ...
    def forward(self, feature):
        ...
        out = F.relu(self.Linear1(feature),inplace=True)   # (N, 512, 32, 40) 
...
class FasterRCNNVGG16(FasterRCNN):
    ...
    def __init__( self, n_fg_class=20, ratios=[1, 2], anchor_scales=[8, 16, 32] ):
        ...
        head = VGG16RoIHead(n_class=n_fg_class + 1,roi_size=[32,40],spatial_scale=(1. / self.feat_stride))
...
class VGG16RoIHead(nn.Module):
    ...
    def __init__(self, n_class, roi_size, spatial_scale):
        ...
        self.roi_size_h = roi_size[0]
        self.roi_size_w = roi_size[1]
        self.spatial_scale = spatial_scale
        self.roi = RoIPooling2D(self.roi_size_h, self.roi_size_w, self.spatial_scale)
