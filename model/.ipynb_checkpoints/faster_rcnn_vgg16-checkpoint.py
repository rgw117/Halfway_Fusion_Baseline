...
class VGG_vis_lwir_feature(nn.Module):
    ...
    def __init__(self):
        ...
        self.pool4_vis = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1, ceil_mode=False)
        ...
        self.pool4_lwir = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1, ceil_mode=False)
    ...
    def forward(self, vis_image, lwir_image):
        ...
        concatenate_feature = F.relu(self.conv1x1(concatenate_feature)) # (N, 512, 32, 40)

        concatenate_feature = F.relu(self.conv5_1(concatenate_feature))  # (N, 512, 32, 40)
        concatenate_feature = F.relu(self.conv5_2(concatenate_feature))  # (N, 512, 32, 40)
        concatenate_feature = F.relu(self.conv5_3(concatenate_feature))  # (N, 512, 32, 40)
        concatenate_feature = self.pool5(concatenate_feature)  # (N, 512, 16, 20)
...
class VGG_vis_lwir_classifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.Linear1 = nn.Linear(in_features=512*16*20, out_features=4096, bias=True)
...
class FasterRCNNVGG16(FasterRCNN):
    ...
    def __init__( self, n_fg_class=1, ratios=[0.5, 1, 2], anchor_scales=[8, 16, 32] ):
        ...
        head = VGG16RoIHead(n_class=n_fg_class + 1,roi_size=[16,20],spatial_scale=(1. / self.feat_stride)) # roi_size = feature map size
